...
**12 个检查模块 · 61 条决策规则 · 18 个场景 · 206 项自动化测试。**

规则由真实 `zen-engine==2.0.2` 执行。无需 AI API key；智能体申请由预设案例和表单模拟生成。邮件、接口、读写与删除由模拟工具返回回执。退款会更新持久化的本机支付沙箱，包括订单可退余额和退款流水，但不调用真实支付服务。
...
1. 选择「小额退款自动执行」，评估并执行一次性授权。
2. 打开「退款沙箱」，查看 `ORD-1001` 的可退余额下降，并产生一笔退款流水。
3. 选择「退款需要主管确认」，体验客服主管审批。
4. 选择「高额退款双人会签」，客服主管与财务都批准后才能执行。
5. 分别尝试「高风险退款人工复核」「超过可退余额」「订单超出退款期限」。

订单天数、退款历史、风险标记和可退余额全部由服务端本机账本提供，浏览器不能伪造。执行入口会在消费一次性授权的同一数据库事务内重新检查余额并写入退款。

### 1. 先看允许与拦截

1. 选择「读取公开资料」，点击「评估这次操作」。
2. 查看十二项检查，点击「模拟执行这次操作」。
...
导入 [rules/agent-approval.json](rules/agent-approval.json) 到 <https://editor.gorules.io/>。这是一张 16 节点的 JDM 决策图（输入、十二项检查、风险汇总、最终决策、输出）。
...
206 项测试覆盖双版本案例、退款资格与审批角色、支付沙箱持久化、执行前余额复核、权限矩阵、额度临界值、非法输入、所有字段的授权绑定、多人会签、过期、规则切换、并发消费、哈希链和 HTTP 接口来源检查。
