...
CHECKS = [
 ...
 ('budget', '成本与额度', [
  ('request.cost > 500',4,'SINGLE_BUDGET','单次预计成本超过 500 演示点数',''),
  ('facts.cost_hour + request.cost > 1000',4,'TOTAL_BUDGET','近一小时累计成本将超过 1,000 演示点数',''),
  ('request.cost > 100',3,'FINANCE_REVIEW','预计成本超过 100 点，需要财务批准','finance'),
  ('request.cost > 0',1,'BUDGET_OK','预计成本在预算范围内',''),
...
def build():
    ...
    nodes.append({'id':'severity','type':'expressionNode','name':'合并风险等级','position':{'x':2700,'y':200},'content':{'passThrough':True,'expressions':[{'id':'maximum','key':'severity','value':'max(['+', '.join('checks.'+key+'.severity' for key,_,_ in CHECKS)+'])'}]}})
    edges.append({'id':previous+'-severity','sourceId':previous,'targetId':'severity','type':'edge'})
    nodes.append({'id':'verdict','type':'decisionTableNode','name':'最终决策','position':{'x':3000,'y':200},'content':{'hitPolicy':'first','passThrough':True,'inputs':[{'id':'severity','name':'最高风险等级','field':'severity'}],'outputs':[{'id':'outcome','name':'决策','field':'outcome'}],'rules':[{'_id':v,'severity':str(n),'outcome':json.dumps(v)} for n,v in [(4,'deny'),(3,'review'),(2,'redact'),(1,'allow')]]}})
    nodes.append({'id':'response','type':'outputNode','name':'审批结论','position':{'x':3300,'y':200}})
